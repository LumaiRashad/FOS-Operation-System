...
void Placement (struct Env * curenv, uint32 fault_va, struct Frame_Info * ptr_frame_info)
{
	allocate_frame(&ptr_frame_info);
			...
			{
				if(USER_HEAP_MAX < fault_va && fault_va < USTACKTOP)
				{
					int ret2 = pf_add_empty_env_page(curenv, fault_va, 1);
					...
					panic ("Page Doesn't Exist in Either Stack or Page File !!");
			}
...
void Placement_FIFO (struct Env * curenv, uint32 fault_va, struct Frame_Info * ptr_frame_info)
{
	allocate_frame(&ptr_frame_info);
	map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va, PERM_USER|PERM_WRITEABLE);
	int ret = pf_read_env_page(curenv, (void*)fault_va);
...
void FIFO_Algorithm (struct Env * curenv, uint32 fault_va, struct Frame_Info * ptr_frame_info, uint32 VictimVA)
{
	uint32 *ptr_page_table =NULL, *Null_Ptr = NULL;
	get_page_table(curenv->env_page_directory, (void *)VictimVA, &ptr_page_table);
	if ((ptr_page_table!=NULL)&&(ptr_page_table[PTX(VictimVA)]&PERM_MODIFIED))
		{
			ptr_frame_info=NULL;
			ptr_frame_info = get_frame_info(curenv->env_page_directory, (void *)VictimVA, &Null_Ptr);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	uint32 WS_curr_size=env_page_ws_get_size(curenv);

	for (int i = 0; i<curenv->page_WS_max_size; i++)//First Scenario
	...
	//PART 1: If WS is not FULL -> Placement
	if(curenv->page_WS_max_size > WS_curr_size)
	{
		//Placement (curenv, fault_va, ptr_frame_info);
		allocate_frame(&ptr_frame_info);
		map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va, PERM_USER|PERM_WRITEABLE);
		int ret = pf_read_env_page(curenv, (void*)fault_va);
		if(ret == E_PAGE_NOT_EXIST_IN_PF)
	...
	//PART 2: If WS is Already FULL -> FIFO Replacement Algorithm + Placement
	else
	{
		uint32 VictimVA =env_page_ws_get_virtual_address(curenv, curenv->page_last_WS_index);
		//FIFO_Algorithm (curenv, fault_va, ptr_frame_info, VictimVA);
		uint32 *ptr_page_table =NULL, *Null_Ptr = NULL;
		get_page_table(curenv->env_page_directory, (void *)VictimVA, &ptr_page_table);
		if ((ptr_page_table!=NULL)&&(ptr_page_table[PTX(VictimVA)]&PERM_MODIFIED))
		{
		...
		ptr_frame_info=NULL;
		//Placement_FIFO (curenv, fault_va, ptr_frame_info);
		allocate_frame(&ptr_frame_info);
		map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va, PERM_USER|PERM_WRITEABLE);
		int ret = pf_read_env_page(curenv, (void*)fault_va);
		if(ret == E_PAGE_NOT_EXIST_IN_PF)
